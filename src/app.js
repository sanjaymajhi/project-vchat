    ...
    all_users.once("value", (e) => {
      ...
      if (req.query.room) {
        ...
        dbRef
          .child("users")
          .child(email)
          .set({ rooms: [email], username: name, password: pass });
      }
...
io.of("/stream").on("connection", (socket) => {
  ...
  socket.on("chat", (data) => {
    ...
    dbRef
      ...
      .child(snd.room)
      .child(snd.timestamp)
      .set({ sender: snd.sendername, message: snd.message, email: snd.email });
  ...
  socket.on("recievedPoll", (data) => {
    ...
    if (
      2 * vote_counts[data.room] + 1 >
      socket.adapter.rooms[data.room].length
    ) {
...
io.of("/user").on("connection", (socket) => {
  ...
  socket.on("chat", (data) => {
    // store in database
    dbRef.child("rooms").child(data.room).child(data.timestamp).set({
    ...
    io.of("/stream")
      .to(data.room)
      .emit("chat", { sender: data.sendername, msg: data.message });
  });
});
